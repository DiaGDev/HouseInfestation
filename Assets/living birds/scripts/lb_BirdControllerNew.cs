...
public class lb_BirdControllerNew : MonoBehaviour
{
    ...
    void SpawnBird()
    {
        if (activeBirds < idealNumberOfBirds && birdIndex < maximumNumberOfBirds)
        {
            // Only spawn birds within a certain distance of the spawn area
            Vector3 spawnPosition = spawnArea.transform.position + Random.insideUnitSphere * unspawnDistance;
            RaycastHit hit;
            if (Physics.Raycast(spawnPosition, Vector3.down, out hit, unspawnDistance, groundLayer))
            {
                GameObject bird = myBirds[birdIndex];
